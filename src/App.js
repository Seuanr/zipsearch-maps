...
function App(props) {
  ...
  const [zipSearch, setZipSearch] = useState(null);
  const [country, setCountry] = useState("SE");

  //This code sends the request to get the zipcodes from Zippopotam
  const getLocation = (e) => {
    e.preventDefault();
    axios(`https://api.zippopotam.us/${country}/${zipSearch}`)
      .then((response) => {
  ...
  const {
    ...
    lastZip = "",
    name = "",
  } = Countries.places.find((c) => c.value === country) || {};
  ...
  return (
    <div className="mainDiv">
      ...
      <form onSubmit={getLocation}>
        <div className="inputDiv">
          ...
          <select
            ...
            value={country}
            onChange={(e) => setCountry(e.target.value) + clearData()}
          >
      ...
      <p>
        In {name} the first zipcode is{" "}
        {firstZip + " and the last is " + lastZip}
      </p>
        ...
        data.places.map((place, i) => (
          <article key={i} className="article">
            <div className="buttonDiv">
              ...
              <button className="deleteBtn" onClick={clearData}>
                Delete
              </button>
            </div>
...
export default GoogleApiWrapper({
  apiKey: "API KEY IS IN THE DEMANDO MESSAGE",
})(App);
